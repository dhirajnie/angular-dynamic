...
export class ResourceCategoriesService {
  ...
  getResourceCategoriesList(query: string, nextIndex: number, paginationSize: number): Observable<any> {
    ...
    return this.http.get(this.appContext.context + PathConstats.resourceCategoriesApi, requestOptions).pipe(
