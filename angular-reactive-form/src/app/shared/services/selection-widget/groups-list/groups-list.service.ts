...
export class GroupsListService {
  ...
  private extractData(res) {
    let body = res;
    if (body != undefined && body != null && body.groups != undefined && body.groups != null) {
      for (let i = 0; i < body.groups.length; ++i) {
        body.groups[i].type = "group";
      }
    }
    return body || {};
  }

  getGroupsList(query: string, nextIndex: number, paginationSize: number): Observable<any> {
    ...
    return this.http.get(this.appContext.context + PathConstats.groupsListApi, requestOptions).pipe(
