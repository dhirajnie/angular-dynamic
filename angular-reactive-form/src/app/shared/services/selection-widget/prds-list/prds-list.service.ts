...
export class PrdsListService {
  ...
  private extractData(res) {
    let body = res;
    if (body != undefined && body != null && body.requestDefs != undefined && body.requestDefs != null) {
      for (let i = 0; i < body.requestDefs.length; ++i) {
        body.requestDefs[i].type = "prd";
      }
    }
    return body || {};
  }

  getPrdsList(query: string, nextIndex: number, paginationSize: number, processType: string): Observable<any> {
    ...
    if (!this.vUtils.isEmptyString(processType)) {
      params = params.set('processType', processType);
    }
    const requestOptions = {
      params: params
    };
    return this.http.get(this.appContext.context + PathConstats.prdsApi, requestOptions).pipe(
